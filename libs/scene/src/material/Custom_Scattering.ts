...
/**
 * CustomScatteringMaterial class
 * @public
 */
export class CustomScatteringMaterial
  ...
  implements Clonable<CustomScatteringMaterial>
{
  /** @internal */
  ...
  /**
   * Creates an instance of CustomScatteringMaterial class
   */
  constructor() {
    super();
    ...
    this.useFeature(CustomScatteringMaterial.FEATURE_VERTEX_NORMAL, true);
    this.useFeature(CustomScatteringMaterial.FEATURE_SUBSURFACE_SCATTERING, true);
  ...
  clone() {
    const other = new CustomScatteringMaterial();
    other.copyFrom(this);
    return other;
  }
  copyFrom(other: this) {
  ...
  get vertexNormal() {
    return this.featureUsed<boolean>(CustomScatteringMaterial.FEATURE_VERTEX_NORMAL);
  }
  set vertexNormal(val) {
    this.useFeature(CustomScatteringMaterial.FEATURE_VERTEX_NORMAL, !!val);
  }
  /** true if vertex normal attribute presents */
  get vertexTangent() {
    return this.featureUsed<boolean>(CustomScatteringMaterial.FEATURE_VERTEX_TANGENT);
  }
  set vertexTangent(val) {
    this.useFeature(CustomScatteringMaterial.FEATURE_VERTEX_TANGENT, !!val);
  }
  /** true if subsurface scattering is enabled */
  get subsurfaceScattering() {
    return this.featureUsed<boolean>(CustomScatteringMaterial.FEATURE_SUBSURFACE_SCATTERING);
  }
  set subsurfaceScattering(val) {
    this.useFeature(CustomScatteringMaterial.FEATURE_SUBSURFACE_SCATTERING, !!val);
  }
  ...
  vertexShader(scope: PBFunctionScope) {
    ...
    if (
      ...
      scope.$outputs.zSubsurfaceColor = this.getInstancedUniform(
        scope,
        CustomScatteringMaterial.SUBSURFACE_COLOR_UNIFORM
      );
      scope.$outputs.zSubsurfaceScale = this.getInstancedUniform(
        scope,
        CustomScatteringMaterial.SUBSURFACE_SCALE_UNIFORM
      );
      scope.$outputs.zSubsurfacePower = this.getInstancedUniform(
        scope,
        CustomScatteringMaterial.SUBSURFACE_POWER_UNIFORM
      );
      scope.$outputs.zSubsurfaceIntensity = this.getInstancedUniform(
        scope,
        CustomScatteringMaterial.SUBSURFACE_INTENSITY_UNIFORM
      );
    }
  }
